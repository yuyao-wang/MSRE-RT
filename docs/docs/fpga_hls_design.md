...
The hardware-oriented source lives in `fpga_emulation/`. It contains HLS kernels,
...
Vivado/Vitis HLS synthesis uses TCL entry points under `fpga_emulation/hls_modules/`.
Example scripts include:

```sh
vitis_hls -f fpga_emulation/hls_modules/hls_core_step_n200_s1_10ns_lowlane.tcl
vitis_hls -f fpga_emulation/hls_modules/hls_bop_step_n200_s1_10ns_lowlane.tcl
```

## Reports

Tracked HLS reports, schedule XML, design XML, and schedule diagrams are under
`docs/synthesis_reports/`. Analysis summaries from board and timing
runs are under `fpga_emulation/analysis_artifacts/`.
