...
hardware-mapping and FPGA-emulation CMake build checks.
...
```sh
python3 -m verification.reactivity_sweep \
  --quick \
  --case-pcm 0,-75 \
  --insertion-time-s 1 \
  --end-time-s 3 \
...
```sh
python3 -m verification.external_validation \
  --nodes 20 \
  --reported-n 20 \
  --steady-steps 1 \
...
python3 -m fpga_emulation.vcu118.msr_vcu118_host --help
python3 -m fpga_emulation.vcu118.msr_transient_batch_vcu118_host --help
```
