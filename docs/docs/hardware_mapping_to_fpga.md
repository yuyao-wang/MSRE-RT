# Hardware Mapping To FPGA Emulation

## Deliverables

- Plain C++ reference solver: [`hardware_mapping/msr_plain.cpp`](../../hardware_mapping/msr_plain.cpp)
- Plain C++ build file: [`hardware_mapping/CMakeLists.txt`](../../hardware_mapping/CMakeLists.txt)
- Vitis HLS kernel-oriented implementation: [`fpga_emulation/msr_vitis_kernel.cpp`](../../fpga_emulation/msr_vitis_kernel.cpp)
- Vitis syntax-check build file: [`fpga_emulation/CMakeLists.txt`](../../fpga_emulation/CMakeLists.txt)
- Experimental full-transient batch top: `msr_transient_batch_kernel()` inside [`fpga_emulation/msr_vitis_kernel.cpp`](../../fpga_emulation/msr_vitis_kernel.cpp)
- Same-init plain C++ transient control runner: [`fpga_emulation/vcu118/msr_transient_batch_plain_timed.cpp`](../../fpga_emulation/vcu118/msr_transient_batch_plain_timed.cpp)
- Resident transient benchmark script: [`fpga_emulation/analyze_transient_batch_bench.py`](../../fpga_emulation/analyze_transient_batch_bench.py)

The hardware-mapping implementation preserves the reference-model step ordering
and physics decomposition:
...
That change is deliberate. The C++17 layer is the deployment-oriented mapping
used for consistency checks; the Vitis HLS layer is the schedulable hardware
form.
...
If a single-kernel HLS implementation is preferred first, keep the exact top-level order already used in [`fpga_emulation/msr_vitis_kernel.cpp`](../../fpga_emulation/msr_vitis_kernel.cpp), then split into two kernels only after profiling.
...
Artifacts for the article are under `fpga_emulation/analysis_artifacts/precursor_analytic_update_20260618_interleaved/`.
