...
FPGA_EMULATION_DIR = REPO_ROOT / "fpga_emulation"
...
from fpga_emulation.analyze_fpga_kernel_run import parse_csynth_summary
from fpga_emulation.vcu118 import msr_vcu118_host
...
def compile_batch_runner(binary_path: Path, hardware_substeps: int, point_kinetics_solver: int) -> None:
    ...
    cmd.extend(
        [
            str(FPGA_EMULATION_DIR / "vcu118" / "msr_transient_batch_sw_timed.cpp"),
            "-o",
            str(binary_path),
        ]
    )
...
def compile_same_init_plain_runner(binary_path: Path, hardware_substeps: int, point_kinetics_solver: int) -> None:
    cmd = [
        ...
        "-DMSR_PRECURSOR_ANALYTIC_UPDATE=0",
        f"-DMSR_POINT_KINETICS_SOLVER={point_kinetics_solver}",
        str(FPGA_EMULATION_DIR / "vcu118" / "msr_transient_batch_plain_timed.cpp"),
        "-o",
        str(binary_path),
    ]
    subprocess.run(cmd, check=True, cwd=REPO_ROOT)
...
def run_plain_cpp(steps: int, insertion_pcm: float, insertion_time_s: float, out_dir: Path) -> dict[str, object]:
    ...
    cmd = [
        str(REPO_ROOT / "hardware_mapping" / "build" / "msr_plain"),
        str(steps),
        str(out_dir),
        str(insertion_pcm),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", type=Path, default=FPGA_EMULATION_DIR / "analysis_artifacts" / "transient_batch_bench_20260618")
