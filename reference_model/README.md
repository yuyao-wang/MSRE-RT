...
```sh
python3 reference_model/main.py --steps 2 --output-dir /tmp/msre_python_smoke --no-plots
```

By default, simulation outputs are written under the checked reference-data
area `verification/simulation_results/`. For smoke runs or new
experiments, pass `--output-dir` to write under `/tmp/...` or an ignored
artifact directory such as `verification/outputs/...` so the Python
...
python3 reference_model/main.py \
  ...
  --output-dir verification/outputs/python_run
```
